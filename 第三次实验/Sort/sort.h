...
private:
    ...
    static int count_move; // 统计关键字移动次数
    static int count_compare; //  统计关键字比较次数
public:
    void timing(vector<T> &data, void (*func)(vector<T> &d), bool show_result, bool check_sorted);

    void timing_complex(void (*func)(vector<T> &d), vector<T> &data_random, vector<T> &data_a, vector<T> &data_de,
...
void Sort<T>::strait_insert_sort(vector<T> &data) {
    ...
    for (i = 1; i < data.size(); ++i) {
        ...
        {
            T temp = data[i];
            for (j = i - 1; j >= 0 && data[j] > temp; --j, ++count_compare) {
